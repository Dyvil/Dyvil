...
public class EnumConstant extends Field
{
	...
	public EnumConstant(IClass enclosingClass, Name name)
	{
		...
		attributes.addFlag(Modifiers.ENUM_CONST);
	}

	public EnumConstant(@NonNull SourcePosition position, Name name, AttributeList attributes)
	{
		this(null, position, name, attributes);
	}

	public EnumConstant(IClass enclosingClass, SourcePosition position, Name name, AttributeList attributes)
	{
		super(enclosingClass, position, name, Types.UNKNOWN, attributes);
		attributes.addFlag(Modifiers.ENUM_CONST);
	}
	...
	{
		final ArgumentList arguments = new ArgumentList(new StringValue(this.name.unqualified),
		                                                new IntValue(this.index));
