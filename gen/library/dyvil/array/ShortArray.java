...
public abstract class ShortArray
{
	...
	{
		int index = 0;
		final short[] array = new short[(int) (to - from + 1)];
		for (; from <= to; from++)
		{
			array[index++] = from;
		}
	...
	{
		int index = 0;
		final short[] array = new short[(int) (toExclusive - from)];
	...
	public static short[] $plus(short[] array, short value)
	{
		...
		final short[] res = new short[size + 1];
	...
	{
		final int size = array.length;
		final Short[] boxed = new Short[size];
		for (int i = 0; i < size; i++)
		{
			boxed[i] = array[i];
		}
