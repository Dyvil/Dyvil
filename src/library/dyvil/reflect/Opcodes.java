...
public interface Opcodes
{
	...
	/**
	 * Removes the first two values from the stack (the first value in case of a {@code long} or {@code double})
	 */
	int POP2 = 88;
	...
	/**
	 * Removes two {@code int} values from the stack, adds them and pushes the result onto the stack.
	 */
	int IADD = 96;

	/**
	 * Removes two {@code long} values from the stack, adds them and pushes the result onto the stack.
	 */
	int LADD = 97;

	/**
	 * Removes two {@code float} values from the stack, adds them and pushes the result onto the stack.
	 */
	int FADD = 98;

	/**
	 * Removes two {@code double} values from the stack, adds them and pushes the result onto the stack.
	 */
	int DADD = 99;

	/**
	 * Removes two {@code int} values from the stack, subtracts them and pushes the result onto the stack.
	 */
	int ISUB = 100;

	/**
	 * Removes two {@code long} values from the stack, subtracts them and pushes the result onto the stack.
	 */
	int LSUB = 101;

	/**
	 * Removes two {@code float} values from the stack, subtracts them and pushes the result onto the stack.
	 */
	int FSUB = 102;

	/**
	 * Removes two {@code double} values from the stack, subtracts them and pushes the result onto the stack.
	 */
	int DSUB = 103;

	/**
	 * Removes two {@code int} values from the stack, multiplies them and pushes the result onto the stack.
	 */
	int IMUL = 104;

	/**
	 * Removes two {@code long} values from the stack, multiplies them and pushes the result onto the stack.
	 */
	int LMUL = 105;

	/**
	 * Removes two {@code float} values from the stack, multiplies them and pushes the result onto the stack.
	 */
	int FMUL = 106;

	/**
	 * Removes two {@code double} values from the stack, multiplies them and pushes the result onto the stack.
	 */
	int DMUL = 107;

	/**
	 * Removes two {@code int} values from the stack, divides them and pushes the result onto the stack.
	 */
	int IDIV = 108;

	/**
	 * Removes two {@code long} values from the stack, divides them and pushes the result onto the stack.
	 */
	int LDIV = 109;

	/**
	 * Removes two {@code float} values from the stack, divides them and pushes the result onto the stack.
	 */
	int FDIV = 110;

	/**
	 * Removes two {@code double} values from the stack, divides them and pushes the result onto the stack.
	 */
	int DDIV = 111;

	/**
	 * Removes two {@code int} values from the stack, calculates the remainder of a division and pushes the result onto
	 * the stack.
	 */
	int IREM = 112;

	/**
	 * Removes two {@code long} values from the stack, calculates the remainder of a division and pushes the result onto
	 * the stack.
	 */
	int LREM = 113;

	/**
	 * Removes two {@code float} values from the stack, calculates the remainder of a division and pushes the result
	 * onto the stack.
	 */
	int FREM = 114;

	/**
	 * Removes two {@code double} values from the stack, calculates the remainder of a division and pushes the result
	 * onto the stack.
	 */
	int DREM = 115;

	/**
	 * Removes an {@code int} value from the stack, negates it and pushes the result onto the stack.
	 */
	int INEG = 116;

	/**
	 * Removes a {@code long} value from the stack, negates it and pushes the result onto the stack.
	 */
	int LNEG = 117;

	/**
	 * Removes a {@code float} value from the stack, negates it and pushes the result onto the stack.
	 */
	int FNEG = 118;

	/**
	 * Removes a {@code double} value from the stack, negates it and pushes the result onto the stack.
	 */
	int DNEG = 119;

	/**
	 * Removes two {@code int} values from the stack, left-shifts value1 by value2 bits and pushes the result onto the
	 * stack.
	 */
	int ISHL = 120;
	/**
	 * Removes two {@code long} values from the stack, left-shifts value1 by value2 bits and pushes the result onto the
	 * stack.
	 */
	int LSHL = 121;

	/**
	 * Removes two {@code int} values from the stack, arithmetic right-shifts value1 by value2 bits and pushes the
	 * result onto the stack.
	 */
	int ISHR = 122;

	/**
	 * Removes two {@code long} values from the stack, arithmetic right-shifts value1 by value2 bits and pushes the
	 * result onto the stack.
	 */
	int LSHR = 123;

	/**
	 * Removes two {@code int} values from the stack, logical right-shifts value1 by value2 bits and pushes the result
	 * onto the stack.
	 */
	int IUSHR = 124;

	/**
	 * Removes two {@code long} values from the stack, logical right-shifts value1 by value2 bits and pushes the result
	 * onto the stack.
	 */
	int LUSHR = 125;

	/**
	 * Removes two {@code int} values from the stack, bitwise ANDs them and pushes the result onto the stack.
	 */
	int IAND = 126;

	/**
	 * Removes two {@code long} values from the stack, bitwise ANDs them and pushes the result onto the stack.
	 */
	int LAND = 127;

	/**
	 * Removes two {@code int} values from the stack, bitwise ORs them and pushes the result onto the stack.
	 */
	int IOR = 128;

	/**
	 * Removes two {@code long} values from the stack, bitwise ORs them and pushes the result onto the stack.
	 */
	int LOR = 129;

	/**
	 * Removes two {@code int} values from the stack, bitwise XORs them and pushes the result onto the stack.
	 */
	int IXOR = 130;

	/**
	 * Removes two {@code long} values from the stack, bitwise XORs them and pushes the result onto the stack.
	 */
	int LXOR = 131;
	...
	/**
	 * Removes an {@code int} value from the stack, converts it to a {@code long} value and pushes the result onto the
	 * stack.
	 */
	int I2L = 133;

	/**
	 * Removes an {@code int} value from the stack, converts it to a {@code float} value and pushes the result onto the
	 * stack.
	 */
	int I2F = 134;

	/**
	 * Removes an {@code int} value from the stack, converts it to a {@code double} value and pushes the result onto the
	 * stack.
	 */
	int I2D = 135;

	/**
	 * Removes a {@code long} value from the stack, converts it to an {@code int} value and pushes the result onto the
	 * stack.
	 */
	int L2I = 136;

	/**
	 * Removes a {@code long} value from the stack, converts it to a {@code float} value and pushes the result onto the
	 * stack.
	 */
	int L2F = 137;

	/**
	 * Removes a {@code long} value from the stack, converts it to a {@code double} value and pushes the result onto the
	 * stack.
	 */
	int L2D = 138;

	/**
	 * Removes a {@code float} value from the stack, converts it to an {@code int} value and pushes the result onto the
	 * stack.
	 */
	int F2I = 139;

	/**
	 * Removes a {@code float} value from the stack, converts it to a {@code long} value and pushes the result onto the
	 * stack.
	 */
	int F2L = 140;

	/**
	 * Removes a {@code float} value from the stack, converts it to a {@code double} value and pushes the result onto
	 * the stack.
	 */
	int F2D = 141;

	/**
	 * Removes a {@code double} value from the stack, converts it to an {@code int} value and pushes the result onto the
	 * stack.
	 */
	int D2I = 142;

	/**
	 * Removes a {@code double} value from the stack, converts it to a {@code long} value and pushes the result onto the
	 * stack.
	 */
	int D2L = 143;

	/**
	 * Removes a {@code double} value from the stack, converts it to a {@code float} value and pushes the result onto
	 * the stack.
	 */
	int D2F = 144;

	/**
	 * Removes an {@code int} value from the stack, converts it to a {@code byte} value and pushes the result onto the
	 * stack.
	 */
	int I2B = 145;

	/**
	 * Removes an {@code int} value from the stack, converts it to a {@code char} value and pushes the result onto the
	 * stack.
	 */
	int I2C = 146;

	/**
	 * Removes an {@code int} value from the stack, converts it to a {@code short} value and pushes the result onto the
	 * stack.
	 */
	int I2S = 147;
	...
	/**
	 * Pushes the instance of the current {@link Intrinsic} method call onto the stack.
	 */
	int LOAD_0 = -1;

	/**
	 * Pushes the first argument of the current {@link Intrinsic} method call onto the stack.
	 */
	int LOAD_1 = -2;

	/**
	 * Pushes the second argument of the current {@link Intrinsic} method call onto the stack.
	 */
	int LOAD_2 = -3;
	...
	/**
	 * Removes a {@code boolean} value from the stack, inverts it and pushes the result back onto the stack.
	 */
	int BNOT = 270;

	/**
	 * Removes an {@code int} value from the stack, bitwise-inverts it and pushes the result back onto the stack.
	 */
	int INOT = 271;

	/**
	 * Removes a {@code long} value from the stack, bitwise-inverts it and pushes the result back onto the stack.
	 */
	int LNOT = 272;
