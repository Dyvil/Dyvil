...
public class FloatHashMap<V>
{
	...
	public FloatHashMap(int size, float loadFactor)
	{
	...
	private static int getHashedKey(float key)
	{
		return hash(Float.floatToIntBits(key));
	}
	...
	protected final Entry<V> getEntry(float key)
	{
		int i = getHashedKey(key);
	...
	public V put(float key, V paramObject)
	{
		int i = getHashedKey(key);
	...
	protected final Entry<V> removeKey(float key)
	{
	...
	private void createKey(int hash, float key, V object, int index)
	{
